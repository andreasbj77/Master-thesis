...
```{r}
arima_test_fitted1 <- cbind(data_test, arima_fc1) %>% rename(truth = V1, pred = x) %>% mutate(error = truth-pred)
arima_test_fitted2 <- cbind(data_test, arima_fc2) %>% rename(truth = V1, pred = x) %>% mutate(error = truth-pred)
arima_test_fitted3 <- cbind(data_test, arima_fc3) %>% rename(truth = V1, pred = x) %>% mutate(error = truth-pred)
arima_test_fitted4 <- cbind(data_test, arima_fc4) %>% rename(truth = V1, pred = x) %>% mutate(error = truth-pred)
```
...
```{r}
arima_test_fitted_features1 <- cbind(data_test, arima_fc_features1) %>% rename(truth = V1, pred = x) %>% mutate(error = truth-pred)
arima_test_fitted_features2 <- cbind(data_test, arima_fc_features2) %>% rename(truth = V1, pred = x) %>% mutate(error = truth-pred)
arima_test_fitted_features3 <- cbind(data_test, arima_fc_features3) %>% rename(truth = V1, pred = x) %>% mutate(error = truth-pred)
arima_test_fitted_features4 <- cbind(data_test, arima_fc_features4) %>% rename(truth = V1, pred = x) %>% mutate(error = truth-pred)
```
...
```{r}
dm.test(arima_test_fitted_features1$error, arima_test_fitted_features3$error, alternative = "greater", h=1, power=2)

dm.test(arima_test_fitted_features1$error, arima_test_fitted4$error, alternative = "greater", h=1, power=2)


dm.test(residuals(arima_refit_features1), residuals(arima_refit_features3), h=1)
```
