...
```{r}
data <- read_csv("https://raw.githubusercontent.com/andreasbj77/Master-thesis/main/Data/DataCreationCarlsberg.csv")
```
...
```{r}
data$Carlsberg %<>% scale(center = FALSE, scale = TRUE)
data$DanskeBank %<>% scale(center = FALSE, scale = TRUE)
data$DSV %<>% scale(center = FALSE, scale = TRUE)
data$GN %<>% scale(center = FALSE, scale = TRUE)
data$JyskeBank %<>% scale(center = FALSE, scale = TRUE)
data$Tryg %<>% scale(center = FALSE, scale = TRUE)
data$OilPrice %<>% scale(center = FALSE, scale = TRUE)

att_Carlsberg <- attr(data$Carlsberg, 'scaled:scale')

data$Carlsberg %<>% as.numeric()
data$DanskeBank %<>% as.numeric()
data$DSV %<>% as.numeric()
data$GN %<>% as.numeric()
data$JyskeBank %<>% as.numeric()
data$Tryg %<>% as.numeric()
...
The variables needed to forecast Carlsberg is selected from the complete data set
```{r}
data_Carlsberg <- data %>% select(Date, Carlsberg)
```
...
```{r}
training <- data_Carlsberg %>% filter(Date <= "2017-12-29")
testing <- data_Carlsberg %>% filter(Date > "2017-12-29" & Date <= "2018-12-28")
```


The data sets are divided into the response and explanatory variables. Here a lead of Carlsberg acts as the response variable.
...
y_train <- training %>%
  select(-Date) %>%
  mutate(Carlsberg = Carlsberg %>% lead(1)) %>%
  mutate(Carlsberg = ifelse(is.na(Carlsberg), lag(Carlsberg, 1), Carlsberg)) %>%
  select(Carlsberg) %>% 
  as.matrix()
...
```{r}
n_input = 15  # amount of timesteps in the model
n_features = ncol(data_Carlsberg) - 1 # amount of explanatory variables
...
tuner <- Hyperband(
  ...
  objective = "val_mape",
  max_epochs = 10,
  project_name = "Carlsberg_hyper_FNN_Single")
```
...
tuner <- Hyperband(
  ...
  objective = "val_mape",
  max_epochs = 30,
  project_name = "Carlsberg_hyper_RNN_Single")
```
...
model <- keras_model_sequential()
  ...
  model %>% layer_lstm(units = hp$Int('units',
                                       min_value =32,
                                       max_value = 1024,
                                       step = 32),
                        input_shape = c(n_input, n_features),
                        activation = hp$Choice("lstm_activation", values = c("relu", "tanh")), return_sequences = FALSE)
  for (i in 1:(hp$Int('num_layers', 1L, 5L)) ) {
     model %>% layer_dense(units = hp$Int(paste0("units",i),
                                       min_value =32,
                                       max_value = 1024,
...
tuner <- Hyperband(
  ...
  objective = "val_mape",
  max_epochs = 30,
  project_name = "Carlsberg_hyper_LSTM_Single")
```
...
model <- keras_model_sequential()
  ...
  model %>% bidirectional(
            layer_lstm(units = hp$Int('units',
                                       ...
                                       step = 32),
                       input_shape = c(n_input, n_features),
                       activation = hp$Choice("lstm_activation",
                                              values = c("relu", "tanh")),
...
tuner <- Hyperband(
  ...
  objective = "val_mape",
  max_epochs = 30,
  project_name = "Carlsberg_hyper_biLSTM_Single")
```
...
model <- keras_model_sequential()
  ...
  model %>% layer_gru(units = hp$Int('units',
                                       min_value =32,
                                       max_value = 1024,
                                       step = 32),
                        input_shape = c(n_input, n_features),
                        activation = hp$Choice("GRU_activation", values = c("relu", "tanh")), return_sequences = FALSE)
  for (i in 1:(hp$Int('num_layers', 1L, 5L)) ) {
     model %>% layer_dense(units = hp$Int(paste0("units",i),
                                       min_value =32,
                                       max_value = 1024,
...
tuner <- Hyperband(
  ...
  objective = "val_mape",
  max_epochs = 30,
  project_name = "Carlsberg_hyper_GRU_Single")
```
...
tuner <- Hyperband(
  ...
  objective = "val_mape",
  max_epochs = 30,
  project_name = "Carlsberg_hyper_biGRU_Single")
```
...
```{r}
ANN_prediction_unscale <- ANN_prediction * att_Carlsberg
RNN_prediction_unscale <- RNN_prediction * att_Carlsberg
LSTM_prediction_unscale <- LSTM_prediction * att_Carlsberg
biLSTM_prediction_unscale <- biLSTM_prediction * att_Carlsberg
GRU_prediction_unscale <- GRU_prediction * att_Carlsberg
biGRU_prediction_unscale <- biGRU_prediction * att_Carlsberg

y_test_unscale <- y_test * att_Carlsberg
```
...
```{r}
save_model_hdf5(model_ANN, "model_ANN_Carlsberg_Single")
save_model_hdf5(model_RNN, "model_RNN_Carlsberg_Single")
save_model_hdf5(model_LSTM, "model_LSTM_Carlsberg_Single")
save_model_hdf5(model_biLSTM, "model_biLSTM_Carlsberg_Single")
save_model_hdf5(model_GRU, "model_GRU_Carlsberg_Single")
save_model_hdf5(model_biGRU, "model_biGRU_Carlsberg_Single")
Results %>% select(Model, RMSE, MAPE) %>% write.csv("Results_Carlsberg_Single")
```
