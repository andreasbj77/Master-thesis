...
```{r}
Two_year_Bond <- read_csv("https://raw.githubusercontent.com/andreasbj77/Master-thesis/main/Data/Denmark%202-Year%20Bond%20Yield%20Historical%20Data.csv") %>% select(Date, Price) %>% rename(TwoYearBond = Price)
```
...
Two_year_Bond %<>% 
  ...
  mutate(Date = Date %>% as_date()) %>%
  mutate(TwoYearBond = TwoYearBond %>% round(2)) %>%
  arrange(Date)
```
...
```{r}
Ten_year_Bond <- read_csv("https://raw.githubusercontent.com/andreasbj77/Master-thesis/main/Data/Denmark-10-Year-Bond-Yield-Historical-Data.csv") %>% select(Date, Price) %>% rename(TenYearBond = Price)
```
...
Ten_year_Bond %<>% 
  mutate(Date = Date %>% parse_date_time(orders = "mdy")) %>%
  mutate(Date = Date %>% as_date()) %>%
  mutate(TenYearBond = TenYearBond %>% round(2)) %>%
  arrange(Date)
```
...
```{r}
Gold_futures <- read_csv("https://raw.githubusercontent.com/andreasbj77/Master-thesis/main/Data/Gold%20Futures%20Historical%20Data.csv") %>% select(Date, Price) %>% rename(GoldFutures = Price)
```
...
Gold_futures %<>% 
  mutate(Date = Date %>% parse_date_time(orders = "mdy")) %>%
  mutate(Date = Date %>% as_date()) %>%
  mutate(GoldFutures = GoldFutures %>% round(2)) %>%
  arrange(Date)
```
...
```{r}
C20_index <- read_csv("https://raw.githubusercontent.com/andreasbj77/Master-thesis/main/Data/OMX%20Copenhagen%2020%20Historical%20Data.csv") %>% select(Date, Price) %>% rename(C20index = Price)
```
...
C20_index %<>% 
  mutate(Date = Date %>% parse_date_time(orders = "mdy")) %>%
  mutate(Date = Date %>% as_date()) %>%
  mutate(C20index = C20index %>% round(2)) %>%
  arrange(Date)
```
